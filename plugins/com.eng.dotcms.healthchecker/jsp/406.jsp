...
<%try{		
	...
	String pointer = (String) com.dotmarketing.cache.VirtualLinksCache.getPathFromCache(host.getHostname() + ":/cms406Page");
	if (!UtilMethods.isSet(pointer)) {
		pointer = (String) com.dotmarketing.cache.VirtualLinksCache.getPathFromCache("/cms406Page");
	}
	
	Logger.debug(this, "cms406Page path is: " + pointer);
...
<head>
	...
	<title><%= LanguageUtil.get(pageContext,"406-page-title") %></title>
...
<div id="main">
	<div id="logo">
		<a href="http://<%=portalUrl%>/"><img src="<%=defaultImage%>" width="140"  hspace="10" border="0" alt="<%=LanguageUtil.get(pageContext,"406-image-title")%>" title="<%=LanguageUtil.get(pageContext,"406-image-title")%>"  /></a>
	</div>
	<div id="text">
	
		<h1><%= LanguageUtil.get(pageContext,"406-title") %></h1>
		
		<p><%= LanguageUtil.get(pageContext,"406-body1") %></p>
		<p><%= LanguageUtil.get(pageContext,"406-body2") %></p>
	</div>
</div>
<br clear="all"/>&nbsp;<br clear="all"/>
<div id="footer">&copy; <script>var d = new Date();document.write(d.getFullYear());</script>, <a href="http://<%=portalUrl%>"><%= LanguageUtil.get(pageContext,"406-copywright") %></a></div>
</body>
</html>
<%} catch( Exception e){
	Logger.error(this, "cms406Page cant display " + e.getMessage(), e);	 %>
403	
