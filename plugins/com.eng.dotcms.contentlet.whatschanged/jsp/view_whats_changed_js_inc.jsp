...
<%boolean canReindex= APILocator.getRoleAPI().doesUserHaveRole(user,APILocator.getRoleAPI().loadRoleByKey(Role.CMS_POWER_USER))|| com.dotmarketing.business.APILocator.getRoleAPI().doesUserHaveRole(user,com.dotmarketing.business.APILocator.getRoleAPI().loadCMSAdminRole());%>
        ...
        function enableShowVersionsButton(identifier) {
        	document.getElementById("selected_contentlet_id").value=identifier;
        	dijit.byId("showVersionsButton").attr("disabled", false);
        }
        ...
        function titleCell (data,text, x) {
            	...
            	ref+=  "<td valign='top'>"
            	ref+=  "<input type=\"radio\" name=\"radio_s_contentlet\" value=\""+identifier+"\" onChange=\"enableShowVersionsButton('"+identifier+"');\"/>&nbsp;&nbsp;"+ text;
            	ref+=   "</td>";
            	ref+=   "</tr></table>";
            	return ref;
        }
    ...
    dojo.addOnLoad(function () {
     ...
     function resetHostValue() {
        ...
        function disableButtonRow() {
         ...
         function showContentletVersions(){
            var form = document.getElementById("search_form");
            var identifier = document.getElementById("selected_contentlet_id").value;
            form.cmd.value = 'show_versions_select';
            form.action = '<portlet:actionURL><portlet:param name="struts_action" value="/ext/contentlet/view_whats_changed_versions" /><portlet:param name="cmd" value="show_versions_select" /></portlet:actionURL>';
            form.action+='&selected_contentlet='+identifier;
            submitForm(form);
        }
