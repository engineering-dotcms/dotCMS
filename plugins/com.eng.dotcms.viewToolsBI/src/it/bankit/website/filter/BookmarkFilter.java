...
public class BookmarkFilter implements Filter {
	...
	ServletException {
		...
		boolean isFile = checkUrl( uri );
		if( UtilMethods.isSet( uri ) && isFile){
			uri = URLDecoder.decode(uri, "UTF-8");
	...
	private boolean checkUrl(String uri) {
		...
		if( UtilMethods.isSet( uri )){
			...
			if( index > 0 ){
				String extens = uri.substring(index+1);
				if( UtilMethods.isSet( extens )  && ( extens.equalsIgnoreCase("pdf") 
						  ||  extens.equalsIgnoreCase("zip")
						  ||  extens.equalsIgnoreCase("ppt")
						  ||  extens.equalsIgnoreCase("pps")
						  ||  extens.equalsIgnoreCase("epub")
						  ||  extens.equalsIgnoreCase("xls")) ) {
					return true;
				}
