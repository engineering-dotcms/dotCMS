...
<%@ include file="/html/plugins/com.eng.dotcms.additions/not_licensed.jsp" %>
...
String linkStructureInode = link.getInode();
String linkSempliceStructureInode = linkSemplice.getInode();
...
function loadTable() {
	...
	var referrer="/c/portal/layout?p_l_id="+lidBL+"&p_p_id=EXT_BKIT_LINK_CHECKER&p_p_action=0&pageNumber="+dojo.byId('currentPage').textContent;
	dojo.empty('table_body');
	var pageSize=25;
	var page=(parseInt(dojo.byId('currentPage').textContent)-1)*pageSize;
	var id = dojo.byId('currentPage');
    if(typeof id.textContent == "undefined"){
            referrer="/c/portal/layout?p_l_id="+lidBL+"&p_p_id=EXT_BKIT_LINK_CHECKER&p_p_action=0&pageNumber="+dojo.byId('currentPage').innerText;
            page=(parseInt(dojo.byId('currentPage').innerText)-1)*pageSize;
    }
	dojo.xhr('GET',{
		url:'/DotAjaxDirector/com.eng.dotcms.additions.ajax.LinkCheckerAjaxAction/cmd/getBrokenLinks/offset/'+page+'/pageSize/'+pageSize+'/structInode/'+dijit.byId('structureSelect').get('value'),
		handleAs: 'json',
		load: function(data) {
...
dojo.ready(function(){
    ...
    resized();
    dijit.byId('structureSelect').set('value', '<%=linkStructureInode%>');
    loadTable();
});
...
<div class="portlet-wrapper">
    ...
    <div class="subNavCrumbTrail">
        <ul id="subNavCrumbUl">        
            <li>
                <%=LanguageUtil.get(pageContext, "javax.portlet.title.EXT_BKIT_LINK_CHECKER")%>
            </li>
            <li class="lastCrumb"><span><%=LanguageUtil.get(pageContext, "javax.portlet.title.EXT_BKIT_LINK_CHECKER_VIEW")%></span></li>
        </ul>
        <div class="clear"></div>
    </div>
